 ...
 */
// PHR
static char cmtbuffer[MAX_CMD_SIZE];
...
inline void get_serial_commands() {
  ...
  inline void get_sdcard_commands() {
    ...
    if (printStarted == true && !card.sdprinting){
      // PHR
      // en test
      // !! semble etre déclanché même lors d'une impression 
      printtime =0;
      tempsEcouleGCODE = 0;
      sdposLastTimeUpdate = 0;
      SERIAL_ECHOLN("card.sdprinting not printing");
      printStarted = false;
      return;
    }
    /**
    ...
    while (commands_in_queue < BUFSIZE && !card_eof && !stop_buffering) {
      ...
      if (card_eof || n == -1
          || sd_char == '\n' || sd_char == '\r'
          || ((sd_char == '#' || sd_char == ':' || (sd_char == ';' && sd_count)) && !sd_comment_mode)
      ) {
        if (card_eof) {
        ...
        // PHR
        // Si sd_count != 0 -> commentaire non vide -> recherche d'indication de temps d'impression 
        if (comment_count){
          cmtbuffer[comment_count] = 0; //terminate string
          String commentL(cmtbuffer);
            SERIAL_ECHO("\r\ncommentaire lu : ");
            SERIAL_ECHOLN(commentL);
          int sep = commentL.indexOf(':');
          if(cmtbuffer[0] == ';' &&  sep != -1){
              String titreLoc = commentL.substring(1,sep);
              String valueLoc = commentL.substring(sep+1);
      ...
      // PHR 
        if (sd_comment_mode){
          if(comment_count < (MAX_CMD_SIZE - 1)){
            cmtbuffer[comment_count++] = sd_char;
          }
        }
      }
      // PHR
...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G29() {
    ...
    if (!isnan(measured_z)) {
        ...
        // PHR
        // moyenne locale pour l'autolevel
        report_current_position();
        const float l = (z_values[0][0] + z_values[1][0] + z_values[2][0] + z_values[0][1] + z_values[1][1] + z_values[2][1] + z_values[0][2] + z_values[1][2] + z_values[2][2]) / 9,
                  v = LOGICAL_TO_NATIVE(l, Z_AXIS),
                  d = current_position[Z_AXIS] - v;
        current_position[Z_AXIS] = d;
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'P': switch (parser.codenum){
      case 0: gcode_P0();break;                     // M0: Unconditional stop, M1: Conditional stop
      default: parser.unknown_command_error();
    }
    break;
